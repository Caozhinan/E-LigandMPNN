...
class RPE_for_LigandMPNN(nn.Module):
    """
    Implements Algorithm 3 in AF3
    """
    ...
    def __init__(self, r_max: int = 32, s_max: int = 2, c_z: int = 128) -> None:
        ...
        # R_idx, chain_labels, R_idx_original
        super(RPE_for_LigandMPNN, self).__init__()
        self.r_max = r_max
        self.s_max = s_max
        self.c_z = c_z
        # self.linear_no_bias = LinearNoBias(
        ...
        # )
        self.linear_no_bias = LinearNoBias(
            in_features=( 2 * self.r_max + 2 ), out_features=self.c_z
        )
        # self.input_feature = {
        #     "asym_id": 1,
        #     "residue_index": 1,
    ...
    def forward(self, input_feature_dict: dict[str, Any]) -> torch.Tensor:
        ...
        #------------------------Next is Revise------------------------
        b_same_chain = (
            input_feature_dict["chain_labels"][..., :, None]
            == input_feature_dict["chain_labels"][..., None, :]
        ).long()  # [..., N_token, N_token]
        b_same_residue = (
            input_feature_dict["R_idx"][..., :, None]
            == input_feature_dict["R_idx"][..., None, :]
        ).long()  # [..., N_token, N_token]

        d_residue = torch.clip(
            input=input_feature_dict["R_idx"][..., :, None]
            - input_feature_dict["R_idx"][..., None, :]
            + self.r_max,
            min=0,
            max=2 * self.r_max, #r_max = 32
        ) * b_same_chain + (1 - b_same_chain) * (
            2 * self.r_max + 1
        )  # [..., N_token, N_token]  #链内的位置算距离，链外的位置默认最大

        a_rel_pos = F.one_hot(d_residue, 2 * (self.r_max + 1))
        # pdb.set_trace()
        # print("RPE_for_LigandMPNN.py:  1")
        # d_token = torch.clip(
        #     input=input_feature_dict["token_index"][..., :, None]
        #     - input_feature_dict["token_index"][..., None, :]
        #     + self.r_max,
        ...
        #     2 * self.r_max + 1
        # )  # [..., N_token, N_token]
        # a_rel_token = F.one_hot(d_token, 2 * (self.r_max + 1))  #因为一个residue可能有多个token，这里忽略掉，这是专门针对多修饰残基的
        ...
        #     2 * self.s_max + 1
        # )  # [..., N_token, N_token]
        # a_rel_chain = F.one_hot(d_chain, 2 * (self.s_max + 1))  #这个是针对entity的，所以也去掉
        ...
        if self.training:
            ...
            #del d_chain, d_token, d_residue, b_same_chain, b_same_residue
            del d_residue, b_same_chain, b_same_residue
            origin_shape = a_rel_pos.shape[:-1]
            Ntoken = a_rel_pos.shape[-2]
            a_rel_pos = a_rel_pos.reshape(-1, a_rel_pos.shape[-1])
            chunk_num = 1 if Ntoken < 3200 else 8
