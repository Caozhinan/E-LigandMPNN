...
class AAE_for_LigandMPNN(nn.Module):
    """
    Implements Algorithm 5 in AF3
    """
    ...
    def __init__(
        ...
        Args:
            ...
            blocks_per_ckpt: number of AtomAttentionEncoder/AtomTransformer blocks in each activation checkpoint
                ...
                is performed.
        """
        super(AAE_for_LigandMPNN, self).__init__()
        self.has_coords = has_coords
        self.c_atom = c_atom
        self.c_atompair = c_atompair
        self.c_token = c_token
        ...
        #     "ref_element": 128,
        #     "ref_atom_name_chars": 4 * 64,
        # } #添加原子的性质，element,xyz37,

        self.input_feature = {
            "atom37_index": 36,
            #"ref_charge": 1,
        } #添加原子的性质, element, xyz37, 后期考虑加上相对位置特征放进来, residue_constraint里面有,距离CA距离的信息也可以加进来
    ...
    def forward(
        ...
        input_feature_dict["atom_to_token_idx"] = input_feature_dict["atom_to_token_idx"].view(-1)

        atom_to_token_idx = input_feature_dict["atom_to_token_idx"]
        # Create the atom single conditioning: Embed per-atom meta data
        # [..., N_atom, C_atom]
        batch_shape = input_feature_dict["xyz_37_valid"].shape[:-2]
        N_atom = input_feature_dict["xyz_37_valid"].shape[-2]
        c_l = self.linear_no_bias_f(
            torch.cat(
                [
                    F.one_hot(input_feature_dict[name], num_classes=36).reshape(
                        *batch_shape, N_atom, self.input_feature[name]
                    ).float()
                    for name in self.input_feature
                ],
        ...
        #     compute_mask=True,
        # )  # 这个可能是关键的一步
        #print("AAE_1:", c_l.shape)
        q_trunked_list, k_trunked_list, pad_info = rearrange_qk_to_dense_trunk(
            q=[input_feature_dict["ref_pos"], input_feature_dict["atom_to_token_idx"]],
            k=[input_feature_dict["ref_pos"], input_feature_dict["atom_to_token_idx"]],
            dim_q=[-2, -1],
            dim_k=[-2, -1],
            n_queries=self.n_queries,
            n_keys=self.n_keys,
        ...
        #print("AAE_2:", q_trunked_list[0].shape, q_trunked_list[1].shape, k_trunked_list[0].shape, k_trunked_list[1].shape)
        ...
        #print("AAE_3:", d_lm.shape, v_lm.shape)
        ...
        if r_l is not None:
            ...
            #print("AAE_6:", q_l.shape, p_lm.shape, z_local_pairs.shape, c_l.shape)
        ...
        #print("AAE_7:", c_l_q.shape, c_l_k.shape )
        if inplace_safe:
            p_lm += self.linear_no_bias_cl(F.relu(c_l_q[..., None, :]))
            p_lm += self.linear_no_bias_cm(F.relu(c_l_k[..., None, :, :]))
            p_lm += self.small_mlp(p_lm)
        ...
        # Cross attention transformer
        #print("AAE_8:", q_l.shape, c_l.shape, p_lm.shape)
        q_l = self.atom_transformer(
            q_l, c_l, p_lm, chunk_size=chunk_size
        )  # [..., (N_sample), N_atom, c_atom]
