...
class LigandMPNNMSELoss(nn.Module):
    """
    [LigandMPNNMSELoss]
    """
    ...
    def __init__(
        ...
        super(LigandMPNNMSELoss, self).__init__()
        self.weight_mse = weight_mse
        self.weight_dna = weight_dna
        self.weight_rna = weight_rna
        self.weight_ligand = weight_ligand
    ...
    def weighted_rigid_align(
        ...
        return (true_coordinate.detach(), weight.detach())
    ...
    def forward(
        ...
        # print("weighted_align_mse_loss", per_sample_weighted_mse)
        # import pdb
        # pdb.set_trace()
        loss = loss_reduction(weighted_align_mse_loss, method=self.reduction)
        return loss
...
class LigandMPNNLoss(nn.Module):
    """Aggregation of the various losses"""

    def __init__(self) -> None:
        super(LigandMPNNLoss, self).__init__()
        #self.configs = configs

        self.alpha_diffusion = 1 #1e-4 #self.configs.loss.weight.alpha_diffusion
        self.alpha_distogram = 3 #3e-2 #self.configs.loss.weight.alpha_distogram
        self.alpha_bond = 0.0  # or 1 in finetuning stages #self.configs.loss.weight.alpha_bond
        self.weight_smooth_lddt = 1.0  # or 0 in finetuning stages # self.configs.loss.weight.smooth_lddt

        self.lddt_radius = {
            "is_nucleotide_threshold": 30.0,
            "is_not_nucleotide_threshold": 10.0, #15.0
        }
        ...
        #self.mse_loss = MSELoss( weight_mse=1/3, weight_dna=5.0, weight_rna=5.0, weight_ligand=10.0, eps=1e-6 )
        #self.mse_loss = LigandMPNNMSELoss( weight_mse=1/3, weight_dna=5.0, weight_rna=5.0, weight_ligand=10.0, eps=1e-6 )
        self.mse_loss = LigandMPNNMSELoss( weight_mse=1.0, weight_dna=5.0, weight_rna=5.0, weight_ligand=10.0, eps=1e-6 )
        self.bond_loss = BondLoss( eps=1e-6 )
        self.smooth_lddt_loss = SmoothLDDTLoss( eps=1e-6 )
        self.distogram_loss = DistogramLoss( min_bin=2.3125, max_bin=21.6875, no_bins=64, eps=1e-6 )
    ...
    def calculate_label(
        ...
        # )  # [..., N_atom, N_atom]
        distance = (cdist(label_dict["x_gt_augment"],label_dict["x_gt_augment"])* distance_mask).to(label_dict["x_gt_augment"].dtype)
        #这部分其实可以改一下, 因为你是有参考坐标的, 不一定只能根据侧链的坐标去更新其他的坐标信息, 这个是很重要的一点.


        coord_mask = label_dict["coordinate_mask"].bool()
        lddt_mask = coord_mask.unsqueeze(2) != coord_mask.unsqueeze(1)

        lddt_mask = compute_lddt_mask(
            true_distance=distance,
            distance_mask=lddt_mask,
        ...
        if not True: #self.configs.loss_metrics_sparse_enable:
    ...
    def calculate_losses(
        ...
        if mode == "train":
            ...
            #print("noise_level", pred_dict["noise_level"])
            if not False: #self.configs.train_confidence_only:
                # Scale diffusion loss with noise-level
                diffusion_per_sample_scale = ((
                    pred_dict["noise_level"] ** 2 + 16.0**2 #self.configs.sigma_data**2
                ) / ( 16.0 * pred_dict["noise_level"]) ** 2 ) ** 0.6 #self.configs.sigma_data
            #print("diffusion_per_sample_scale", diffusion_per_sample_scale)
        ...
        if False and mode == "train": #self.configs.train_confidence_only 
            # Skip Diffusion Loss and distogram loss
            loss_fns = {}
        else:
            # Diffusion Loss: SmoothLDDTLoss / BondLoss / MSELoss
            loss_fns = {}
            if True: #self.configs.loss.diffusion_lddt_loss_dense:
                loss_fns.update(
                    {
                        "smooth_lddt_loss": lambda: self.smooth_lddt_loss.dense_forward_for_MPNN(
                            pred_coordinate=pred_dict["coordinate"],
                            true_coordinate=label_dict["x_gt_augment"], #label_dict["coordinate"],
                            lddt_mask=label_dict["lddt_mask"],
                            diffusion_chunk_size=1, #self.configs.loss.diffusion_lddt_chunk_size,
                        )  # it's faster is not OOM
                    }
                )
            elif True: #self.configs.loss.diffusion_sparse_loss_enable:
                loss_fns.update(
                    {
            ...
            loss_fns.update(
                {
                    "bond_loss": lambda: (
                        ...
                        if True #self.configs.loss.diffusion_sparse_loss_enable
                        else self.bond_loss(
                            pred_distance=pred_dict["distance"],
                            true_distance=label_dict["distance"],
                            distance_mask=label_dict["distance_mask"],
                            bond_mask=feat_dict["bond_mask"],
                            per_sample_scale=diffusion_per_sample_scale,
                            diffusion_chunk_size=1, #self.configs.loss.diffusion_bond_chunk_size,
                        )
                    ),
    ...
    def forward(
        ...
        """
        diffusion_chunk_size = -1 #self.configs.loss.diffusion_chunk_size_outer
        assert mode in ["train", "eval", "inference"]
        # Pre-computations
        with torch.no_grad():
        ...
        if diffusion_chunk_size <= 0:
            ...
            elif False: #self.configs.train_confidence_only:
