...
def broadcast_token_to_atom(
    ...
    if len(atom_to_token_idx.shape) == 1 or sum(1 for s in atom_to_token_idx.shape if s != 1) == 1:
        return x_token[..., atom_to_token_idx.view(-1), :]
    else:
        assert atom_to_token_idx.shape[:-1] == x_token.shape[:-2]
        # try:
        #     assert atom_to_token_idx.shape[:-1] == x_token.shape[:-2]
        # except AssertionError:
        #     print(f"[Error] Shape mismatch in broadcast_token_to_atom:")
        #     print(f"  atom_to_token_idx.shape: {atom_to_token_idx.shape}")
        #     print(f"  x_token.shape: {x_token.shape}")
        #     raise  # 可选：继续抛出错误终止运行，方便调试
