...
#     # Check if path is StringIO, if so, write to temporary file
#     if isinstance(path, io.StringIO):
#         with tempfile.NamedTemporaryFile(delete=False, suffix=".pdb") as temp_file:
#             temp_file.write(path.getvalue().encode())  # Write StringIO content to temp file
#             temp_file_path = temp_file.name  # Store temp file path
#     else:
#         assert os.path.exists(path), f"PDB file not found: {path}"
#         temp_file_path = path  # If it's a file path, use it directly
...
#                 plddts = extract_plddt(temp_file_path)
...
def get_struc_seq(foldseek,
    ...
    Args:
        foldseek: Binary executable file of foldseek
        path: Path to pdb file or StringIO object containing pdb data
        chains: Chains to be extracted from pdb file. If None, all chains will be extracted.
        process_id: Process ID for temporary files. This is used for parallel processing.
        plddt_mask: If True, mask regions with plddt < plddt_threshold. plddt scores are from the pdb file.
        plddt_threshold: Threshold for plddt. If plddt is lower than this value, the structure will be masked.
    ...
    Returns:
        seq_dict: A dict of structural seqs. The keys are chain IDs. The values are tuples of (seq, struc_seq, combined_seq).
    """
    assert os.path.exists(foldseek), f"Foldseek not found: {foldseek}"
    ...
    tmp_save_path = f"get_struc_seq_{process_id}_{time.time()}.tsv"
    if foldseek_verbose:
        cmd = f"{foldseek} structureto3didescriptor --threads 1 --chain-name-mode 1 {temp_file_path} {tmp_save_path}"
    else:
        cmd = f"{foldseek} structureto3didescriptor -v 0 --threads 1 --chain-name-mode 1 {temp_file_path} {tmp_save_path}"
    ...
    # Remove temporary pdb file if we created one
    if isinstance(path, io.StringIO):
