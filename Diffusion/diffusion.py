...
from protenix.model.modules.embedders import FourierEmbedding, RelativePositionEncoding, RPE_for_LigandMPNN
...
from Diffusion.utils import expand_at_dim
from protenix.openfold_local.model.primitives import LayerNorm
from Diffusion.utils import get_checkpoint_fn
import pdb
...
class DiffusionConditioning(nn.Module):
    ...
    def __init__(
        ...
        # Line1-Line3:
        self.relpe = RPE_for_LigandMPNN(c_z=c_z)
...
class DiffusionModule(nn.Module):
    ...
    def __init__(
        ...
        self.atom_attention_encoder = AAE_for_LigandMPNN(
            **atom_encoder,
            c_atom=c_atom,
    ...
    def f_forward(
        ...
        # print("blocks_per_ckpt", blocks_per_ckpt)
        # print("self.use_fine_grained_checkpoint", self.use_fine_grained_checkpoint)
    ...
    def forward( #forward函数里的内容修改一下，主要结合mask进行修改。
        self,
        ...
        B, C, N, _ = x_noisy.shape
        noise_mask_expand = noise_mask.unsqueeze(1).expand(B, C, N, 1)
        noise_level_sq = (t_hat_noise_level ** 2).view(B, C, 1, 1)
        noise_level_sq = noise_level_sq.expand(B, C, N, 1)
        sigma_data_sq = self.sigma_data ** 2
        sigma_sq_total = torch.where(
            ~noise_mask_expand.bool(),  # 位置无噪声
            torch.full_like(noise_level_sq, fill_value=sigma_data_sq),
            sigma_data_sq + noise_level_sq
        )
        r_noisy = x_noisy / torch.sqrt(sigma_sq_total)
        ...
        # x_denoised = noise_mask * x_denoised_full + (1 - noise_mask) * x_noisy
        #plan 1 END

        #plan 2
        s_ratio = (t_hat_noise_level / self.sigma_data)[..., None, None]  # [B, C, 1, 1]
        s_ratio = s_ratio.expand(B, C, N, 1)  # 广播到 [B, C, N, 1]



        # 把 s_ratio 对于未加噪的部分替换为 0（即 sigma / sigma = 1 → s_ratio = 0）
        # 相当于使用 sigma_data，还原比例为 1
        s_ratio = torch.where(~noise_mask_expand.bool(), torch.zeros_like(s_ratio), s_ratio)

        # 构建还原公式
        c_skip = 1 / (1 + s_ratio ** 2)
        c_out = t_hat_noise_level[..., None, None] * noise_mask_expand / torch.sqrt(1 + s_ratio ** 2)

        x_denoised = (c_skip * x_noisy + c_out * r_update).to(r_update.dtype)
        #pdb.set_trace()
        #后续训练的差不多了在这里检测一下试试，如果有必要，再考虑让模型学会根据noise控制不同原子的偏移程度。
